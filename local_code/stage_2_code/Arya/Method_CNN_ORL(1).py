'''
Concrete CNN method class for ORL face classification.
40 classes, input shape (1, 112, 92).
'''
...
class CNN_ORL_Net(nn.Module):
    """
    CNN for 112x92 single-channel face images.

    Architecture
    ============
    Conv(1→32,  3x3, pad=1) -> BN -> ReLU -> MaxPool(2x2)   [56x46]
    Conv(32→64, 3x3, pad=1) -> BN -> ReLU -> MaxPool(2x2)   [28x23]
    Conv(64→128,3x3, pad=1) -> BN -> ReLU -> MaxPool(2x2)   [14x11]
    Flatten -> FC(128*14*11 -> 256) -> ReLU -> Dropout(0.5)
    FC(256 -> 40)
    """
...
class Method_CNN_ORL:
    ...
    def __init__(self,
        ...
        self.model     = CNN_ORL_Net(num_classes).to(self.device)
    ...
    def train(self, X_train, y_train, X_test, y_test):
        ...
        print(f'\n[Method_CNN_ORL] Training on {self.device}')
