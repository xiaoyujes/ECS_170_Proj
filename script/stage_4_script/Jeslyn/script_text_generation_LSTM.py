...
from local_code.stage_4_code.text_generation.Jeslyn.Method_LSTM import Method_LSTM
...
if 1:
    ...
    result_obj.result_destination_file_name = 'generated_LSTM_'
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('LSTM Text Generation Loss Curve')
    plt.savefig(
        result_obj.result_destination_folder_path +
        result_obj.result_destination_file_name +
        'text_generation_LSTM_learning_curve.png'
    )
    plt.show()
